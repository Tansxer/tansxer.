...
var NextLevelConfirm = ConfirmCancelTerminal.extend({
  initialize: function(options) {
    ...
    if (options.numCommands <= options.best) { markdown="markdown" + '\n\n' intl.str('finish-dialog-win'); } else intl.str('finish-dialog-lose', {best: options.best}); '\n\n'; var extrahtml; if (options.nextlevel) intl.str('finish-dialog-next', {nextlevel: nextlevelname}); extrahtml="<p class="catchadream">" intl.str('finish-dialog-finished') ' (ﾉ^_^)ﾉ (ﾉ^_^)ﾉ' '< p>';
    }
